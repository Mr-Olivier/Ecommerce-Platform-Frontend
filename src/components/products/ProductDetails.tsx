...
//   // Enhanced add to cart function with proper API integration
...
//     try {
//       if (token) {
//         // User is logged in - make API call first
//         console.log("User is logged in, making API call");
//         await handleAuthenticatedAddToCart(token);
//       } else {
//         // Guest user - add to local cart only
...
//       // If API call fails but user is logged in, fall back to local cart
//       if (token) {
...
//   // Handle add to cart for authenticated users (with API call)
//   const handleAuthenticatedAddToCart = async (token: string) => {
//     try {
//       const response = await axios.post(
//         "http://localhost:4000/api/cart/items",
//         {
...
//       if (response.data.status === "success") {
//         // Check if this is a guest response (product validation only)
//         if (response.data.data.guest) {
//           console.log("API returned guest response, adding to local cart");
//           handleGuestAddToCart();
//           return;
//         }

//         // Authenticated user - cart was saved to database
//         console.log("Cart saved to database, refreshing cart");
...
//       if (axios.isAxiosError(error) && error.response?.status === 401) {
//         console.log(
//           "Authentication error, clearing token and adding to local cart"
//         );
//         localStorage.removeItem("token");
//         localStorage.removeItem("user");
//         handleGuestAddToCart();
...
//   // Handle add to cart for guest users (local storage only)
...
//       // Create cart item for local storage
//       const cartItem = {
//         id: `guest_${Date.now()}_${product.id}`, // Unique ID for guest cart
//         productId: product.id,
//         name: product.name,
//         price:
//           typeof product.price === "string"
...
//       // Add to cart context (this will save to localStorage)
//       addItem(cartItem);
...
//           <button
//             type="button"
//             className="bg-indigo-600 hover:bg-indigo-700 text-white w-full px-6 py-3 rounded-lg mt-4 flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//             onClick={handleAddToCart}
//             disabled={isAdding || product.stock === 0}
//           >
//             {isAdding ? (
...
//             {localStorage.getItem("token")
//               ? "Item will be saved to your account"
//               : "No account required - sign in at checkout"}
//           </p>
//         </div>
//       </div>
  ...
  const handleAuthenticatedAddToCart = async (token: string) => {
    try {
      ...
      if (response.data.status === "success") {
        ...
        if (response.data.data.guest) {
          console.log(
            "API returned guest response (product validated), adding to local cart"
          );
          handleGuestAddToCart();
          return;
        }

        // Authenticated user - check if cart data is returned
        if (response.data.data.cart && response.data.data.cart.items) {
          ...
          // API succeeded but no cart data returned, fall back to local cart
          console.log(
            "API succeeded but no cart data returned, adding to local cart"
          );
          handleGuestAddToCart();
        }
      } else {
