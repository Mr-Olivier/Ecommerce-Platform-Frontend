...
const PaymentForm: React.FC<PaymentFormProps> = ({ prevStep }) => {
  const { confirmPayment, isLoading, error, clientSecret, orderId } =
    useCheckout();
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      // In a real application, you would use the card details to create a payment method
      // and then confirm the payment with the payment intent client secret

      // For this example, we'll simulate a payment intent ID
      const paymentIntentId =
        clientSecret?.split("_secret_")[0] || "mock_payment_intent_id";

      // Confirm the payment with the backend
      const result = await confirmPayment(paymentIntentId);

      if (result.success) {
        // If payment is successful, navigate to confirmation
